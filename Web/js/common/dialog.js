 ...
 * */
;
(function (win, doc, $, DH, factory) {
    if (typeof define === 'function' && define.amd) {
        define(['jQuery'], function ($j) {
            return factory(win, doc, $j, DH)
        })
    } else if (typeof exports === 'object') {
        module.exports = factory(win, doc, $, DH)
    } else {
        win.dialog = factory(win, doc, $, DH)
    }
}(window, document, jQuery, DH, function (window, document, $, space) {
    'use strict'
    ...
    var dialog = (function () {
        ...
        function renderModal(params) {
            var defaults = {
                    ...
                    cancel: space.language.common.btn.cancel, // 取消按钮
                    ok: space.language.common.btn.ok, // 确认按钮
                },
            ...
            var str = '<div class="modal ' + opts.sizes + '">\
                    ...
                    <div class="modal-content">\
                        ...
                        <div class="modal-footer">\
                            <button type="button" class="default" modal="cancel">' + opts.cancel + '</button>\
                            <button type="button" class="ok" modal="ok">' + opts.ok + '</button>\
                        </div>\
                    </div>\
                </div>'
            return $(str)
        ...
        function getDom(ele) {
            ...
            if (ele == '#confirm') {
                ...
                $ele.attr('id', 'confirm').css('z-index', '102').find('.modal-body').html('<div class="modal-tip-box" style="margin-top: 10px;"><span class="modal-tip">')
            }
            if (ele == '#toast') {
