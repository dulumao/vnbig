...
(function () {
    ...
    function init() {
        ...
        btn.$fsubmitBtn.click(function () {
            ...
            if (pass == '') {
                ...
                ele.$fError.text(DH.language.error.complicacy)
                ele.$fError.removeClass('visibility')
                return
            }
            if (rePass == '') {
            ...
            util.comAjax({
                ...
                if (res.ReturnCode == 1000) {
                    ...
                    // 跳登录页面
                    setTimeout(function () {
                        window.location.href = '/login.html?t=0'
                    }, 1500)
                } else {
    ...
    function getCode() {
        ...
        // var type = 6
        if (ableCode) {
            mail = $.trim(ele.$fmail.val())
            if (mail == '') {
                ele.$fError.text(DH.language.error.emailNulll)
                ele.$fError.removeClass('visibility')
                return
            }
            if (!mailtest.test(mail) && !codetest.test(mail)) {
