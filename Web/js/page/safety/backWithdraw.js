...
(function (window, document, space) {
    ...
    space.backWithdrawInit = function ($html) {
        var dom = initDom()
        // 修改密码输入框的type
            ...
            util.comAjax({
                ...
                if (res.ReturnCode == 1000) {
                    if (!res.Data.TiXianPassResult) {
                        // 未设置提现密码时弹窗之后跳设置提现密码页面
                        DH.error(lang.safety.backWithdraw.prompt[0], function () {
                            history.pushState(state, '', state.url + '?modle=withdraw')
                            getNewPage('withdraw')
                            $("li[srcurl='withdraw']").addClass('selected').siblings().removeClass('selected')
                        })// "请先设置您的提现密码!"
                    }                    
                }
            }).fail(function (err) {
        ...
        function getCode() {
            ...
            if (ableCode) {
                ...
                // console.log(tel);
                if (!tel) {
                    dom.prompt.html(lang.safety.backWithdraw.prompt[1])// "手机号/邮箱号不能为空！"
                    return
                }
                ...
                util.comAjax({
                    ...
                    json: true,
                    type: 'POST',
                    target: dom.verificationCode,
                    data: {
                        MemberKey: DH.UID,
                        Phone: tel,
                        CountryCode: '',
                        SmsType: 10, // 找回提现密码发码 此值为‘10’
                    },
                }).done(function (res) {
        ...
        function behavior() {
            ...
            if (!data.Password) {
                dom.prompt.html(lang.safety.backWithdraw.prompt[2])// "新提现密码不能为空"
                return
            }
            ...
            if (!data.RePassword) {
                dom.prompt.html(lang.safety.backWithdraw.prompt[3])// "重复的新密码不能为空"
                return
            }
            if (data.Password != data.RePassword) {
                dom.prompt.html(lang.safety.backWithdraw.prompt[4])// "两次密码请输入一致"
                return
            }
            backPassword(data)// 请求找回提现密码
        };
        // 向后台发起找回提现密码请求
        function backPassword(data) {
            ...
            util.comAjax({
                ...
                if (res.ReturnCode == 1000) {
                    // console.log(res);
                    dom.input.val('')
                    DH.error(lang.safety.backWithdraw.prompt[5], function () {
        ...
        function setLang() {
            ...
            setText('title', lang.safety.backWithdraw.title)
            setText('SBitems', lang.safety.backWithdraw.SBitems)
            setText('Btn', lang.safety.backWithdraw.Btn)
        }
    }
}(window, document, DH))
