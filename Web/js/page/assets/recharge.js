...
(function (window, document, space) {
    ...
    space.rechargeInit = function () {
        ...
        function historyList(curPage) {
            ...
            util.comAjax({
                ...
                if (res.ReturnCode == 1000) {
                    var data = res.Data.RechargeList,
                        html = historyHtml(data)
                    totalPage = Math.ceil(res.Data.Count / pageSizeInit)
                    if (pageIndex < totalPage) {
                        dom.dataShow.show().html('<span class="btn-loading"><i></i></span><span class="lang-loadLang" index="0">' + space.language.asset.loadHint[0] + '</span>')
                    }
                    if (pageIndex == totalPage && totalPage != 1) {
                        dom.dataShow.show().html(space.language.asset.loadHint[1])
                    }
                    if (data && data != '') {
                        if (!curPage || curPage == '1') {
                            dom.history.find('tbody').html(html)
                            pageIndex = 1 // 将当前的页码重置为‘1’，防止滚动以后再操作按钮的‘提交’事件
                        } else {
                            dom.history.find('tbody').append(html)
                        }
                    } else {
                        dom.history.find('tbody').html('<tr><td class="odd" colspan="3">' + space.language.common.noData + '</td> </tr>')
                    }
                } else {
                    space.error(res.ReturnMsg)
                }
        ...
        dom.history.find('.recharge-centre').mCustomScrollbar({ // '充值记录'滚动条设置
            theme: 'dark',
            callbacks: {
